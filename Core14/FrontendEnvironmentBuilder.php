 ...
 * container exclusively when the running TYPO3 major version is 14 (see
 ...
 * frontend builder.
 *
 * @internal Concrete, TYPO3 v14 specific implementation of {@see EnvironmentBuilderInterface}. Resolved
...
final class FrontendEnvironmentBuilder implements EnvironmentBuilderInterface
{
    ...
    public function build(StateBuildContext $stateBuildContext): StateInterface
    {
        ...
        $state = $this->createFrontendRenderingState($stateBuildContext, $state, $site, $siteLanguage);
        return $state;
    }
    ...
    private function createFrontendRenderingState(StateBuildContext $stateBuildContext, State $state, Site $site, SiteLanguage $siteLanguage): State
    {
        ...
        $this->initializePageRenderer($request, $siteLanguage);
        $this->initializeContentObjectRenderer($request, $pageInformation);
        return $state
            ->withRequest($request)
            ->withContext($context);
    }
    ...
    /**
     * Initializes the PageRenderer language, which TYPO3 v13 did through
     * `TypoScriptFrontendController::initializePageRenderer()`. TYPO3 v14 does this in the
     * `PrepareTypoScriptFrontendRendering` middleware, which this mirrors.
     *
     * The template file is intentionally not set: unlike TYPO3 v13, the TYPO3 v14 PageRenderer
     * already defaults to the correct template.
     */
    private function initializePageRenderer(ServerRequestInterface $request, SiteLanguage $siteLanguage): void
    {
        $locales = GeneralUtility::makeInstance(Locales::class);
        $locale = $siteLanguage->hasCustomTypo3Language()
            ? $locales->createLocale($siteLanguage->getTypo3Language())
            : $locales->createLocale((string)$siteLanguage->getLocale());
        GeneralUtility::makeInstance(PageRenderer::class)->setLanguage($locale, $request);
    }

    /**
     * Creates and starts the ContentObjectRenderer, which TYPO3 v13 did through
     * `TypoScriptFrontendController::newCObj()`. TYPO3 v14 creates it without the removed
     * TypoScriptFrontendController constructor argument, as the frontend RequestHandler does.
     */
    private function initializeContentObjectRenderer(ServerRequestInterface $request, PageInformation $pageInformation): void
    {
